...
export class Reader {
		...
		try {
			...
			if (isYouTube) {
				...
				if (iframe && youtubeVideoElement) {
					...
					// YouTube's JS may keep resetting attributes �?					// use a MutationObserver to enforce our settings
	...
	private static registerSelectionToHighlightButton(doc: Document) {
		...
		// Preserve the selection when the pointer goes down on the button �?		// otherwise the browser clears it before click handlers run.
